...
def _project_record(repo_root: Path, slug: str) -> dict[str, Any]:
    try:
        return project_record(repo_root, slug)
...
def _restore_destination(repo_root: Path, relative: PurePosixPath) -> Path:
    """Resolve a restore target without following a repository-local symlink parent."""
    ...
    if not _inside(destination, repo_root):
        raise BundleError(
            f"Restore target leaves repository root: {relative.as_posix()}"
        )
    return destination
...
def _validate_manifest(
    ...
    try:
        normalized_project = normalize_project_record(
            project, source="bundle manifest project"
        )
    except RegistryError as exc:
        raise BundleError(str(exc)) from exc
    slug = normalized_project["path"]
    ...
    return normalized_project, validated
...
def restore_bundle(
    ...
    *,
    ...
    project, entries = _validate_manifest(manifest)
    slug = project["path"]
    try:
        existing_project = find_project(repo_root, slug)
    except RegistryError as exc:
        raise BundleError(str(exc)) from exc
    if existing_project is not None and existing_project != project:
        raise BundleError(
            "Bundle project record conflicts with the local registry; no files were written"
        )
    registry_action = "unchanged" if existing_project is not None else "create"

    actions: list[tuple[str, dict[str, Any], Path]] = []
    conflicts: list[str] = []
    for entry in entries:
        relative = _validate_relative_path(entry["path"])
        destination = _restore_destination(repo_root, relative)
        if destination.exists():
    ...
    if registry_action == "create":
        try:
            add_local_project(repo_root, project)
        except RegistryError as exc:
            raise BundleError(
                f"Payload restored but the local project registry could not be updated: {exc}"
            ) from exc
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    export.add_argument("project", help="project path from the merged public/local registry")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "restore":
            ...
            print(f"Local registry: {summary['registry']}")
