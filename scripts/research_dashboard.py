...
class DashboardData:
    ...
    def registry(self) -> list[dict[str, Any]]:
        try:
            data = load_registry(self.repo_root)
        except RegistryError as exc:
            raise DashboardError(f"Cannot read project registry: {exc}") from exc
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _static(self, name: str) -> None:
        ...
        allowed_file = relative.as_posix() in {"index.html", "app.js", "styles.css"}
    ...
    def _dispatch(self) -> None:
        ...
        if parsed.path in {"/", "/index.html"} and not self._authorized(query):
            ...
            self.end_headers()
            return
        if parsed.path in {"/", "/index.html", "/app.js", "/styles.css"} or parsed.path.startswith("/vendor/katex/"):
