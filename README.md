...
每次开始一轮新的形式化任务，Work 必须先回显并固定启动契约；以前一次的许可和一句笼统的“继续”都不能代替本轮选择。契约至少包括：精确命题、权威来源版本与位置、预期最终 Lean 声明；验收档位；使用普通 Work 运行还是持久 goal；是否允许子代理；以及现有工具链能否运行、缺失工具链能否安装。若允许子代理，还必须逐个角色确定当前环境实际提供的模型、推理强度、任务、读写范围、总数与并发上限、最多追问轮数、整合与最终审计责任人和停止条件。任何字段未定，只能做只读定位，不能写形式化计划、修改代码、运行 Lean/Lake、设置 goal 或委派。这里的“一轮”是同一冻结契约下的一项形式化任务，不是每条编译命令；契约不变时不重复询问。
